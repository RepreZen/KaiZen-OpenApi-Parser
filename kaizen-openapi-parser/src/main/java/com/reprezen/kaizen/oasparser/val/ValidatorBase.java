...
public abstract class ValidatorBase<T> implements Validator<T> {
	...
	// TODO Q: Any help from spec in being able to validate URLs with vars?
	// E.g is our treatment here valid? We
	// assume vars can only appear where simple text can appear, so handling
	// vars means relacing {.*} with "1" and
	// testing for URL validity. We use a digit instead of a letter because
	// it covers vars in the port, and
	// elsewhere digits are always allowed where letters are.
	String origUrl = url;
	if (allowVars) {
	    url = url.replaceAll("\\{[^}]+\\}", "1");
	    if (url.startsWith("1:")) {
		// "1" is not a valid scheme name, so we need to replace it with
		// special scheme, for which we provide a
